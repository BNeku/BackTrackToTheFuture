...
//8. Este predicado indica que hay un elemento en la secuencia igual a uno dado
...
//9. Escribir un predicado que indique si una posicion de una secuencia es la primera posicion de la 
...
predicate esPrimerCero(s : seq<int>, k : int) 
{
	...
	&& s[k] == 0)
	||
	(forall i:: 0 <= i < |s| ==> s[i] != 0 && k == |s|)
	//Te pongo solo esta forma, de la otra forma que me explicaste no recuerdo bien como era
   //Escribe aqui el predicado
   /*
   (exists u:int :: 0 <= u < k <|s| && s[u] != 0 && s[k] ==0 )
   ||
   (forall i :: 0 <= i <=k < |s| && s[i]!=0 && s[k]== |s|)
  // exists u:int :: (0<=u< |s| && s[u] == 0 && forall i:: 0<=i<u ==> s[i]!= 0)
  // || (0<=u < |s| && s[u] == s && forall i:: 0<=i< |s| ==> s[i]!=0)
  */                     
}
...
//{
	//Para dafny si escribes {} quiere decir que te dan igual los duplicados
	// ahora como es [] los duplicados si importan entonces el tamaño de ambos deben de ser iguales

	//(//forall i:: 0<=i< |s| ==> (exists j:int :: 0 <= j < |t| && s[i] == t[j]))
	//habia pensado este predicado de arriba pero me da mal
//}
